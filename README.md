...
| G9 | Hold `N` → tap `A` | `na` | N auto-repeats (with ghosts `.`/`/`), A tap breaks the repeat | Auto-repeat BREAKS when A tapped | ⚠️ **Unaffected key breaks affected auto-repeat.** A is not on any ghost column, yet tapping it stops N's auto-repeat. The controller may re-evaluate the column state on any new keypress event. |
...
9. **Unaffected key breaks affected auto-repeat.** G9 (Hold N → tap A) shows that tapping A — a completely unaffected key on an unrelated column — breaks N's auto-repeat. This is unexpected because A has no connection to the bridge, ghost columns, or column C7. The most likely explanation: the controller re-evaluates the entire matrix state on any new keypress event, and re-scanning the affected column during this re-evaluation triggers the anti-ghosting check differently when additional keys are active. This suggests the fast-typing glitch mechanism may involve the controller's state-change processing, not just the electrical bridge itself.

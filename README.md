...
- **Symptom spread to additional keys** (latest update): extra key presses are now appearing on the **Right arrow** key and further keys beyond the original 6/Y/H/N/Space column cluster. This indicates the conductive residue bridge is no longer confined to the previously identified Cx/Cy/Cz column group — either the contamination has wicked further along the FPC ribbon to additional trace pairs, or progressive acid corrosion has lowered the resistance of previously sub-threshold bridges to the point where the controller now detects them. This is a significant escalation: the fault is actively spreading, which increases the urgency of ultrasonic cleaning and raises the probability that permanent trace damage is accumulating.
...
| **[Service Center Assessment](docs/service-center-assessment.md)** | Service center's "non-serviceable key blocks" position, drying analysis showing inadequate cleaning/drying of the ZIF connector, and second service center's claim that keyboard cannot be detached for ultrasonic cleaning (with technical rebuttal) |
...
**Corrosion status: progressing — new symptoms appeared.** Extra key presses are now occurring on the **Right arrow** key and additional keys beyond the original Cx/Cy/Cz column group. This means the bridge is no longer confined to the three previously identified columns — contamination has either wicked further along the FPC ribbon or progressive acid corrosion has lowered additional bridge resistances below the detection threshold. The damage is actively getting worse, increasing the urgency of ultrasonic cleaning and raising the risk of permanent copper trace damage if further delayed.
...
**Recommended action:** The second service center **cleaned the JT200 ZIF connector** (IPA clean of connector pads and FPC contact fingers) and the keyboard issue **remained unchanged** after cleaning. This definitively rules out the connector contact surfaces as the contamination site. The residue bridge is in the less accessible zones: the FPC ribbon trace gaps (~0.1 mm between polyimide layers) and/or under the sealed key switch bodies (sub-0.3 mm capillary spaces). Manual ZIF connector cleaning has been tried and did not help — proceed directly to **ultrasonic cleaning** to address residue in the FPC trace gaps or under sealed key switches. The second service center has also claimed they cannot ultrasonically clean the keyboard because the keyboard panel is integrated into the top case; this claim is technically inaccurate — the top case with keyboard can be ultrasonically cleaned as a unit after disconnecting the FPC from the logic board, or the FPC ribbon can be cleaned independently. Their alternative proposal (tearing off the keyboard panel and replacing it on screws) is non-standard, destructive to the top case, and does not guarantee resolution of the electrical fault. See [Second Service Center assessment](docs/service-center-assessment.md#second-service-center-keyboard-not-detachable--cannot-ultrasonically-clean) for full analysis. The test results confirm localised, stable contamination — the bridge resistance is stable and the humidity-driven reversibility confirms the residue is still predominantly organic/ionic rather than permanent trace damage, giving ultrasonic cleaning a good probability of success.
